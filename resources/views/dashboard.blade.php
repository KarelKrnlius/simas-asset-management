    ...
    @if(Auth::user()->role_id == 1)
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm h-full">
                ...
                <div id="chartAnalytics" class="min-h-[400px]"></div>
            </div>
                        ...
                        <div class="space-y-6">
                            @php
                                ...
                                if (class_exists('\App\Models\Peminjaman')) {
                                    $myLoans = \App\Models\Peminjaman::where('user_id', Auth::id())
                            ...
                            <div class="flex items-center gap-4 border-b border-slate-200 pb-4 last:border-0">
                                ...
                                <div>
                                    <p class="text-[11px] font-black text-slate-900 uppercase italic">{{ $loan->asset->name }}</p>
                                    <p class="text-[9px] text-slate-400 font-bold uppercase mt-1">Kembali: {{ \Carbon\Carbon::parse($loan->tanggal_kembali)->format('d M') }}</p>
                                </div>
                            </div>
                            @empty
...
<script>
    ...
    var options = {
        ...
        chart: { type: 'bar', height: 400, toolbar: {show: false}, fontFamily: 'Plus Jakarta Sans' },
        plotOptions: { bar: { borderRadius: 12, columnWidth: '40%', distributed: true } },
        colors: ['#ef4444', '#0f172a', '#ef4444', '#0f172a', '#ef4444', '#0f172a', '#ef4444'],
        dataLabels: { enabled: false },
        xaxis: { 
            categories: ['Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab', 'Min'],
            labels: { style: { fontWeight: 800, fontSize: '10px' } }
        },
